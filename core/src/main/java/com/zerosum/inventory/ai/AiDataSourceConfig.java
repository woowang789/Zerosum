 ...
 *
 * <p>클래스 전체를 {@code zerosum.ai.read.url}·{@code zerosum.ai.proposer.url} 둘 다 있을 때만 켠다 —
 * AI를 쓰지 않는 진입점(예: :core만 물고 뜨는 REST API)은 이 프로퍼티 없이도 컨텍스트가 떠야 한다.
 * 끄면 이 네 빈이 통째로 사라지고, {@code @Qualifier("aiReadJdbcClient"/"aiProposerJdbcClient")}로 이
 * 빈들을 주입받는 {@link com.zerosum.inventory.repository.AiQueryRepository}·
 * {@link com.zerosum.inventory.repository.AiProposalRepository}도 주입 대상을 못 찾아 컨텍스트가 깨진다
 * (의도된 동작 — 이 둘도 AI 기능의 일부다). 그 아래로 {@code AiQueryRepository}를 쓰는
 * {@link com.zerosum.inventory.ai.AiQueryService}, {@code AiProposalRepository}를 쓰는
 * {@link com.zerosum.inventory.ai.AiAnalysisService}와
 * {@link com.zerosum.inventory.proposal.ProposalCreationService}까지 연쇄적으로 함께 빠진다 — 즉 AI
 * 조회·분석·제안 생성 전체가 이 프로퍼티 하나로 묶여 있다. 그래서 둘 중 하나만 켜는 것은 의미가 없어
 * name에 둘 다 넣었다(AND) — 켜려면 ai_ro·ai_proposer 계정 설정이 함께 있어야 한다.
 */
@Configuration
@ConditionalOnProperty(prefix = "zerosum.ai", name = {"read.url", "proposer.url"})
