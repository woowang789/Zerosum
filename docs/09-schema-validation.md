...
> **하네스와 애플리케이션이 갈리는 지점.** 위 "거절된다"는 하네스(`tst_count_start`)의 동작이다. 하네스는 호출자가 멱등 키를 건네는 일반 계약을 그대로 구현하므로 두 번째 시작이 제약에 부딪혀 거절된다. 반면 애플리케이션의 실사 시작은 멱등 기록을 쓰지 않고 `location.count_session_id`를 `FOR UPDATE`로 읽어, 열린 세션이 있으면 **그 세션 id를 돌려준다**([실사와 판매 가능 수량](05-count-session.md#시작)). 이유는 그쪽에 적었다 — 대상을 멱등 키로 삼으면 두 번째 실사가 첫 번째의 재생이 되기 때문이다.
>
> **같은 이유로 갈린 곳이 둘 더 있다.** 하네스의 `tst_post`에는 (a) 줄이 하나도 없는 커맨드를 거절하는 검사와 (b) 거래 유형별 가상 로케이션 방향 규칙이 없다. 애플리케이션은 둘 다 `PostingCommand.validate()`에서 막는다([쓰기 경로](04-write-path.md#커맨드-검증)). 두 검사 모두 **스키마로 표현되지 않는 커맨드 검증**이라 하네스가 검증할 대상이 아니다 — 하네스가 보는 것은 제약·트리거·인덱스·권한이 지키는 것이고, 그 범위에서는 여전히 정확하다. 같은 판단의 선례가 `ORPHAN_CONSUME`에도 있다.
>
> 검증하는 것이 서로 다르므로 둘 다 유효하다. 하네스가 보는 것은 **스키마가 무엇을 막는가**(부분 유니크 인덱스가 진행 중 세션 둘을 허용하지 않는다)이고, 그 사실은 애플리케이션 경로에서도 그대로다 — 앱이 인덱스에 닿기 전에 먼저 막을 뿐이다. 애플리케이션 쪽 증거는 `CountSessionLifecycleTest`가 원장에 직접 INSERT해 따로 남긴다.
...
**검증 하네스** — 유지한다. Java 구현이 들어오면 포스팅 로직은 중복되지만, 스키마만 바꿨을 때 Java 컴파일 없이 몇 초 만에 전부 다시 돌릴 수 있다(단언은 그 뒤로 늘어 지금 192건이다). 유스케이스 시나리오는 [테스트 전략](08-testing-roadmap.md#테스트-전략)대로 Testcontainers 테스트로 옮긴다.
