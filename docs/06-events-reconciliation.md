...
불일치가 나오면 자동으로 보정하지 않는다. `inventory_issue`에 기록하고 알림을 보낸 뒤, 원인을 확인하고 바로잡는다. 무엇으로 바로잡을지는 불일치의 성격에 따라 다르다.

①(잔액 투영)·②(할당)은 원장은 맞는데 그 위에 얹은 요약(잔액·할당 카운트)이 잘못 계산된 경우다. 여기에 조정 거래를 쓰면 실제로는 없던 물리적 수량 변화가 원장에 새로 생겨버린다 — 버그는 요약 쪽에 있는데 원장까지 오염시키는 셈이다. 그래서 ①·②는 투영 복구(요약을 원장에 맞춰 다시 계산)로 고친다.

③(원장 체인 끊김)은 조정 거래로 고칠 수 없다. 이미 기록된 `on_hand_after`는 `app_rw`가 UPDATE할 권한 자체가 없고, 권한이 있다 해도 고쳐서는 안 된다 — 그 값 자체가 무엇이 잘못됐는지 보여주는 증거이기 때문이다. ③은 원인을 코드로 고친 뒤 `resolution_note`에 조사 결과를 남기고 거래 없이 종결한다.

조정 거래가 들어맞는 경우는 물리적 차이 — 실사에서 드러난 실제 수량 차이 — 뿐이다. 이때는 원장에 없던 수량이 생기는 게 아니라, 실제로 있었는데 원장에 없던 수량을 뒤늦게 반영하는 것이기 때문이다.

자동 보정은 버그의 흔적을 지워서 같은 문제가 반복되게 만든다.
